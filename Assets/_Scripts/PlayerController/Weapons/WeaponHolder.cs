...
public class WeaponHolder : MonoBehaviour
{
    ...
    {
        // Стрельба
        if (Input.GetMouseButton(0) && currentWeapon && Time.time >= currentWeapon.nextTimeToFire && !rightHolder)  // для левого холдера
        {
        ...
        if (Input.GetMouseButton(1) && currentWeapon && Time.time >= currentWeapon.nextTimeToFire && rightHolder)   // для правого холдера
        {
            currentWeapon.nextTimeToFire = Time.time + 1f / currentWeapon.fireRate;
            currentWeapon.Fire();                                                           // вызываем функцию стрельбы у текущего оружия
        }
        ...
        if (Input.GetAxis("Mouse ScrollWheel") > 0f && rightHolder)                         // управление колёсиком (для правого холдера)
        {
        ...
        if (Input.GetAxis("Mouse ScrollWheel") < 0f && !rightHolder)                        // управление колёсиком (для левого холдера)
        {
            if (selectedWeapon <= 0)
